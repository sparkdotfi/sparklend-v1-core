...
import { CalculateInterestRatesParams } from '../libraries/types/DataTypes.sol';
 ...
 *           - An instance of this same contract, can't be used across different Aave markets, due
 *             to the caching of the PoolAddressesProvider
 */
contract DefaultReserveInterestRateStrategy is IDefaultInterestRateStrategy {
    ...
    // Slope of the stable interest curve when usage ratio > 0 and <= OPTIMAL_USAGE_RATIO. Expressed
    // in ray
    ...
    // Additional premium applied to stable rate when stable debt surpass
     ...
     * @param baseStableRateOffset          The premium on top of variable rate for base stable
     *                                      borrowing rate
     * @param stableRateExcessOffset        The premium on top of stable rate when there stable debt
     *                                      surpass the threshold
     * @param optimalStableToTotalDebtRatio The optimal stable debt to total debt ratio of the
     *                                      reserve
     */
    constructor(
        address provider,
