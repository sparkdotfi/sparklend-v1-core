 ...
 * @dev    Once the PriceOracle gets up after an outage/downtime, users can make their positions
 *         healthy during a grace period. So the PriceOracle is considered completely up once its up
...
contract PriceOracleSentinel is IPriceOracleSentinel {
     ...
     */
    constructor(address provider, address oracle, uint256 gracePeriod) {
     ...
     * @return isUp True if the SequencerOracle is up and the grace period passed, false otherwise
     */
    function _isUpAndGracePeriodPassed() internal view returns (bool) {
        ( , int256 answer, , uint256 lastUpdateTimestamp, ) =
            ISequencerOracle(_sequencerOracle).latestRoundData();

        return answer == 0 && block.timestamp - lastUpdateTimestamp > _gracePeriod;
    }

    /// @inheritdoc IPriceOracleSentinel
    function setSequencerOracle(address sequencerOracle) public onlyPoolAdmin {
        emit SequencerOracleUpdated(_sequencerOracle = sequencerOracle);
    }

    /// @inheritdoc IPriceOracleSentinel
    function setGracePeriod(uint256 gracePeriod) public onlyRiskOrPoolAdmins {
        emit GracePeriodUpdated(_gracePeriod = gracePeriod);
    }

    /// @inheritdoc IPriceOracleSentinel
    function getSequencerOracle() public view returns (address) {
        return _sequencerOracle;
    }
